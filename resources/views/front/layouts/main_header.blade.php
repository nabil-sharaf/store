            ...
            <div class="container">
                <div class="row">
                    <div class="col-xs-12 col-sm-5 col-md-4 col-lg-6">
                        <div
                            class="header-info-left d-flex align-items-center justify-content-center justify-content-sm-start">
                            <p class="mb-0 mr-3">{!! __('main_header.free_returns_shipping')  !!} </p>

                            <!-- Dropdown for language selection -->
                            <div class="dropdown lang-dropdown-link">
                                <a href="#" class="dropdown-toggle" id="languageDropdown" data-bs-toggle="dropdown"
                                   aria-expanded="false">
                    ...
                    <div class="col-xs-12 col-sm-7 col-md-8 col-lg-6 sm-pl-0 xs-pl-15 header-top-right">
                        <div class="header-info">
                            <a href="#"><i class="fa fa-phone"></i> {!!  \App\Models\Admin\Setting::getValue('phone')!!}
                            </a>
                            <a href="{{route('home.contact')}}"> <i
                                    class="fa fa-envelope"></i>{!!  \App\Models\Admin\Setting::getValue('email') !!}</a>

                            @if(auth()->check())
                                <!-- Dropdown for account information -->
                                <a href="#" class="dropdown-toggle account-link" id="accountDropdown"
                                ...
                                <ul class="dropdown-menu text-center" aria-labelledby="accountDropdown">
                                    <li>
                                        <a class="dropdown-item my-header-dropdown-item"
                                           href="{{route('profile.index')}}">{!!  __('main_header.account_settings') !!}</a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item my-header-dropdown-item" href="#"
                            ...
                            <div class="header-align-left">
                                <div class="header-logo-area">
                                    <a href="{{route('home.index')}}">
                                        <img class="logo-main"
                                             src="{{asset('storage/'. ($siteImages?->logo ?: $siteImages?->default_image))}}"
                                             alt="Logo-11"/>
                                ...
                                <div class="header-search-box">
                                    <form action="{{ route('product.search') }}" method="GET">
                                        <div class="form-input-item">
                                            <label for="search"
                                                   class="sr-only">{{ __('main_header.search_placeholder') }}</label>
                                            <input type="text" id="search" name="search"
                                                   placeholder="{{ __('main_header.search_placeholder') }}">
                                    ...
                                    <div class="header-action-wishlist">
                                        @if(auth()->check())
                                            <button class="btn-wishlist"
                                                    ...
                                                    title="{{ __('main_header.wishlist') }}">
                                                <i class="pe-7s-like"></i>
                                            </button>
                                        @else
                                            <button class="btn-wishlist" onclick="wishListMessage(event)"
                                                    title="{{ __('main_header.wishlist') }}">
                                                <i class="pe-7s-like"></i>
                                            </button>
                                        @endif
                            ...
                            <form action="{{ route('product.search') }}" method="GET">
                                <div class="form-input-item">
                                    <label for="search-mobile"
                                           class="sr-only">{{ __('main_header.search_placeholder') }}</label>
                                    <input type="text" id="search-mobile" name="search"
                                           placeholder="{{ __('main_header.search_placeholder') }}">
                                    <button type="submit" class="btn-src">
                                        <i class="pe-7s-search"></i>
                                    </button>
                ...
                <div class="row row-gutter-0 align-items-center">
                    <div class="col-4 col-sm-6 col-lg-2">
                        <div class="header-logo-area">
                            <a href="{{route('home.index')}}">
                                <img class="logo-main"
                                     src="{{asset('storage/'.($siteImages?->logo ?: $siteImages?->defalult_image))}}"
                                     alt="Logo"/>
                                <img class="logo-light"
                                     src="{{asset('storage/'.($siteImages?->logo ?: $siteImages?->defalult_image))}}"
                                     alt="Logo"/>
                    ...
                    <div class="col-lg-8 sticky-md-none">
                        <div class="header-navigation-area d-none d-md-block">
                            <ul class="main-menu nav position-relative">
                                <li><a class="ml--2" href="{{route('home.index')}}">{{ __('main_header.home') }}</a>
                                </li>
                                ...
                                @if(auth()->user())
                                    <form id='logout-form' method="POST" action="{{ route('logout') }}">
                                        ...
                                        <button class="btn-login" type="submit"
                                                title="{{ __('main_header.logout_btn') }}">
                                            <i class="pe-7s-delete-user"></i>
                                        </button>
                                    </form>
                                @else
                                    <button class="btn-login" title="{{ __('main_header.login_btn') }}"
